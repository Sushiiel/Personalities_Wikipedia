...
secrets_path = "/Users/mymac/Desktop/Project_1/wiki_scraper/streamlit/secrets.toml"
secrets = toml.load(secrets_path)
os.environ["COHERE_API_KEY"] = secrets["COHERE_API_KEY"]

def upload_to_s3(file_path, s3_key): s3_client.upload_file(file_path, S3_BUCKET, s3_key)
def delete_from_s3(s3_key): s3_client.delete_object(Bucket=S3_BUCKET, Key=s3_key)

os.environ["STREAMLIT_WATCHER_TYPE"] = "none"
vector_space_dir = os.path.join(os.getcwd(), "vector_db")
if not os.path.exists(vector_space_dir): os.mkdir(vector_space_dir)

st.set_page_config(page_title="Wikipedia RAG ChatBot", layout="centered")
st.title("Wikipedia RAG ChatBot (Scrapy + LangChain + Cohere)")

if 'vectorstore' not in st.session_state: st.session_state['vectorstore'] = None
if 'memory' not in st.session_state: st.session_state['memory'] = ConversationBufferMemory(memory_key="chat_history", return_messages=True)
...
if st.button("Fetch Wikipedia & Create Chatbot"):
    if not first_name:
        ...
        with st.spinner("Running Scrapy spider..."):
            try:
                subprocess.run(["/opt/homebrew/bin/python3.10", "/Users/mymac/Desktop/Project_1/scrapy_runner.py", person_name], check=True)
                ...
                st.error("Scrapy spider failed. Check terminal output.")
